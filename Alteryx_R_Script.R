...
rpmInstall <- function(newPkgs, configPath) {
  ...
  if (length(newPkgs) == 0) {
    stop.Alteryx("All of the provided packages have already been installed.")
  }
  versions <- names(obj$rVersion)
...
rpmUpdate <- function(updatePkgs = NULL, configPath) {
  obj <- loadPkgJSON(configPath)
  versions <- names(obj$rVersion)
  pkgDelta <- list()
...
# State is managed via an external JSON object. This allows us to track state changes between
# Alteryx runs. As a result, the script starts by defining the path to the config object and all
# subsequenct functions take that path as an argument so they can read/write the config object

if ('%Question.radio_rpmInit%') {
  # Create package.json
  args <- read.Alteryx("#1", mode="list")
  configSavePath <- normalizePath(as.character(args[[1]]))
  repoSavePath <- normalizePath(as.character(args[[2]]))
  cranRepoURL <- '%Question.textBox_rpmInit_cranRepoURL%'
  pkgList <- newLinesToChar('%Question.textBox_rpmInit_masterPkgList%')
  pkgType <- '%Question.dropDown_rpmInit_pkgType%'
  versions <- newLinesToChar('%Question.listBox_rpmInit_RVersion%', TRUE)
  rpmInit(repoSavePath, cranRepoURL, pkgList, pkgType, versions, configSavePath)
} else {
  # Perform some RPM action using the provided package.json
  # configPath <- as.character(read.Alteryx("#1", mode="list")[[1]])
  args <- read.Alteryx("#1", mode="list")
  configPath <- normalizePath(as.character(args[[1]]))
  if ('%Question.radio_buildRepo%') {
    buildRepo(NULL, configPath)
  } else if ('%Question.radio_rpmInstall%') {
    pkgList <- newLinesToChar('%Question.textBox_rpmInstall_pkgList%')
    rpmInstall(pkgList, configPath)
  } else if ('%Question.radio_rpmUpdate%') {
    rpmUpdate(NULL, configPath)
  } else {
   stop("No action selected")
  }
}
